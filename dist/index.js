...
var color = __toESM(require_picocolors());
var getChunksForCompressOrDecompressProofs = (command, mmData, proofsToReplace, unifier) => {
  const chunks = [];
  chunks.push(
    mmData.substring(
      0,
...
var compressOrDecompressProofs = (command, mmData, proofIds, all, config) => {
  const completeConfig = applyDefaultsToConfig(config);
